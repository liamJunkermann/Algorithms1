...
class DoublyLinkedList<T extends Comparable<T>> {
    ...
    public void insertBefore(int pos, T data) {
        if (isEmpty()) {
            ...
            // Insert at front
            DLLNode temp = head;
            DLLNode first = new DLLNode(data, null, temp);
            temp.prev = first;
            head = first;
        } else {
            int i = 0;
            for (DLLNode selectedNode = head; selectedNode != null; selectedNode = selectedNode.next) {
                if (i == pos) {
                    DLLNode newNode = new DLLNode(data, selectedNode.prev, selectedNode);
            ...
            // Add data to end
            DLLNode temp = tail;
            DLLNode last = new DLLNode(data, temp, null);
            temp.next = last;
            tail = last;
        }
        return;
    ...
    public boolean deleteAt(int pos) {
        int i = 0;
        for (DLLNode selectedNode = head; selectedNode != null; selectedNode = selectedNode.next) {
            if (pos == 0 && selectedNode.next == null && selectedNode.prev == null) { // Single Element
                head = null;
                tail = null;
                return true;
            } else if (pos == 0) { // First Element in bigger list
                head = head.next;
                head.prev = null;
                return true;
            } else if (i == pos) { // If index has reached pos
                if (selectedNode.next != null) { // Not at end
     ...
     * Worst-case asymptotic running time cost: Theta(N)
     *
     * Justification: 1 loop
     */
    public void reverse() {
        DLLNode temp = null;
        DLLNode selectedNode = head;
     ...
     *         Worst-case asymptotic running time cost: Theta(1)
     *
     *         Justification: Only runs one function
     */
    public T pop() {
        if (isEmpty())
            return null;
     ...
     *             Worst-case asymptotic running time cost: Theta(1)
     *
     *             Justification: Worst case for interior function
     */
